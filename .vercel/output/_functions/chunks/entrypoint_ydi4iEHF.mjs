import { c as createActionsProxy, p as pipelineSymbol, A as AstroError, a as ActionCalledFromServerError, b as cspAlgorithmSchema, d as cspHashSchema, e as allowedDirectivesSchema, f as prependForwardSlash, r as removeTrailingForwardSlash, g as appendForwardSlash, P as Pipeline, h as createStylesheetElementSet, i as createModuleScriptElement, j as findRouteToRewrite, k as createConsoleLogger, l as AstroMiddleware, m as PagesHandler, F as FetchState, n as removeLeadingForwardSlash, o as getParts, q as getPattern, s as createKey, S as SlotString, N as NOOP_MIDDLEWARE_FN, t as renderHead, u as renderTemplate, v as defineAction, w as ActionError } from './entrypoint_yAU3LSnf.mjs';
...
function resolveDirAsUrl(dir, root) {
  let resolvedDir = nodePath.resolve(root, dir);
  if (!resolvedDir.endsWith(nodePath.sep)) {
    resolvedDir += nodePath.sep;
  }
    ...
    build: z.object({
      ...
      client: z.string().optional().default(ASTRO_CONFIG_DEFAULTS.build.client).transform((val) => {
        ...
        return resolveDirAsUrl(
          val,
          nodePath.resolve(fileProtocolRoot, ASTRO_CONFIG_DEFAULTS.outDir)
        );
      }),
...
const $$AdminMailContact = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$AdminMailContact;
  const { fullName, subject, message, email, phone } = Astro2.props;
  const nameWeb = "Cenote Adventuring";
  return renderTemplate`<html lang="es"> <head><meta content="text/html; charset=UTF-8" http-equiv="Content-Type"><meta name="x-apple-disable-message-reformatting">${renderHead()}</head> <body style="background-color:#DFF2FE;font-family:-apple-system,BlinkMacSystemFont,&quot;Segoe UI&quot;,Roboto,Oxygen-Sans,Ubuntu,Cantarell,&quot;Helvetica Neue&quot;,sans-serif"> <table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="max-width:37.5em;padding:20px;border:1px solid rgb(0,0,0, 0.1);border-radius:10px;background-color:#ffffff;margin-top:20px;margin-bottom:20px;"> <tbody> <tr> <td> <table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="overflow:hidden"> <tbody> <tr> <td> <table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="padding:20px;padding-bottom:0"> <tbody style="width:100%"> <tr style="width:100%"> <td data-id="__react-email-column"> <h1 style="font-size:32px;font-weight:bold;text-align:center;">
Mex On The Beach - Web
</h1> <h2 style="font-size:20px;font-weight:bold;text-align:center">
Información del contacto
</h2> <p style="font-size:16px;line-height:24px;margin:16px 0;margin-top:-5px"> <b>Nombre:</b> ${fullName} </p> <p style="font-size:16px;line-height:24px;margin:16px 0;margin-top:-5px"> <b>Correo:</b> ${email} </p> <p style="font-size:16px;line-height:24px;margin:16px 0;margin-top:-5px"> <b>Número Telefónico:</b> ${phone} </p> <p style="font-size:16px;line-height:24px;margin:16px 0;margin-top:-5px"> <b>Asunto:</b> <br> ${subject} </p> <p style="font-size:16px;line-height:24px;margin:16px 0;margin-top:-5px"> <b>Mensaje:</b> <br> ${message} </p> </td> </tr> </tbody> </table> </td> </tr> </tbody> </table> <p style="font-size:12px;line-height:24px;margin:16px 0;text-align:center;color:rgb(0,0,0, 0.7)">
© ${(/* @__PURE__ */ new Date()).getFullYear()} | ${nameWeb} | Todos los derechos reservados.
</p> </td> </tr> </tbody> </table> </body></html>`;
}, "C:/mexonthebeach/src/components/sections/AdminMailContact.astro", void 0);
